...
- **CI/CD**: Cloud Build で git push → 自動デプロイ
...
| `PROJECT_NAME` | リソース名プレフィックス | `rubese` |
...
git push (main)
  │
  ├─ Nginx ビルド ──┐
  │ (並列)          ├─ Artifact Registry push ─→ Cloud Run 更新
  └─ PHP-FPM ビルド ┘     (並列)                  （両コンテナ同時）
```
...
1. GCP コンソール → Cloud Build → トリガー → **リポジトリを接続**
2. GitHub リポジトリを選択して接続
3. `terraform/.env` に以下を追加:
   ```
   GITHUB_OWNER=your-github-username
   GITHUB_REPO_NAME=your-repo-name
   ```
4. `./scripts/setup.sh apply dev` でトリガーが自動作成される
...
```bash
gcloud run services update-traffic rubese-dev-app \
...
| ファイル | 内容 |
|---|---|
| `architecture_*.md` | 全体アーキテクチャ仕様書（全 18 セクション） |
| `docker-deep-dive_*.md` | Docker 構成深堀りガイド（チューニング、最適化、制約） |
| `SOW-*.md` | 作業履歴・問題と対応の記録 |
