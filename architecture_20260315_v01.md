...
**{prefix}** は環境ごとに異なる: `rubese-dev` / `rubese-prod`
...
| 項目 | dev | prod |
|---|---|---|
| 名前 | `rubese-dev-subnet` | `rubese-prod-subnet` |
...
| 項目 | dev | prod |
|---|---|---|
| IP範囲名 | `rubese-dev-google-managed-services` | `rubese-prod-google-managed-services` |
...
| 項目 | dev | prod |
|---|---|---|
| 名前 | `rubese-dev-db` | `rubese-prod-db` |
...
| 項目 | dev | prod |
|---|---|---|
| サービス名 | `rubese-dev-app` | `rubese-prod-app` |
| リージョン | asia-northeast1 | asia-northeast1 |
...
> **判断:** 現在の rubese プロジェクトの規模（PHPアプリ、チーム少人数）では、マイクロサービスは過剰。モジュラーモノリスで**将来の分離を見据えた境界**を設けつつ、運用はシンプルに保つ。
...
> **既存テーブルの扱い:** 既存の rubese DB テーブルは段階的にモジュールに割り当てる。初期は全テーブルを `Shared` 扱いとし、機能単位でモジュールに移行する。
...
| 設定 | dev | prod |
|---|---|---|
| リソース名プレフィックス | `rubese-dev-*` | `rubese-prod-*` |
...
|---|---|---|---|
| VPC | `{prefix}-vpc` | `rubese-dev-vpc` | `rubese-prod-vpc` |
| サブネット | `{prefix}-subnet` | `rubese-dev-subnet` | `rubese-prod-subnet` |
| Cloud SQL | `{prefix}-db` | `rubese-dev-db` | `rubese-prod-db` |
| Cloud Run | `{prefix}-app` | `rubese-dev-app` | `rubese-prod-app` |
| Serverless NEG | `{prefix}-neg` | `rubese-dev-neg` | `rubese-prod-neg` |
| LB IP | `{prefix}-lb-ip` | `rubese-dev-lb-ip` | `rubese-prod-lb-ip` |
| Cloud Armor | `{prefix}-armor-policy` | `rubese-dev-armor-policy` | `rubese-prod-armor-policy` |
| Artifact Registry | `{prefix}-docker` | `rubese-dev-docker` | `rubese-prod-docker` |
| サービスアカウント | `{prefix}-run-sa` | `rubese-dev-run-sa` | `rubese-prod-run-sa` |
| Secret Manager | `{prefix}-db-password` | `rubese-dev-db-password` | `rubese-prod-db-password` |
